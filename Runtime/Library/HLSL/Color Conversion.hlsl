...
// --- FLOAT PRECISION ---
void RgbToHcv_float(in float3 RGB, out float3 HCV)
{
    half4 P = lerp(float4(RGB.bg, -1.0, 2.0/3.0), float4(RGB.gb, 0.0, -1.0/3.0), step(RGB.b, RGB.g));
...
void RgbToHsl_float(in float3 RGB, out float3 HSL)
{
    float3 HCV;
    RgbToHcv_float(RGB, HCV);
...
void HslToRgb_float(in float3 HSL, out float3 RGB)
{
    HSL = float3(frac(HSL.x), clamp(HSL.yz, 0.0, 1.0));
    const float3 rgbRaw = clamp(abs(fmod(HSL.x * 6.0 + float3(0.0, 4.0, 2.0), 6.0) - 3.0) - 1.0, 0.0, 1.0);
